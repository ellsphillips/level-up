...
export default function PathwayContent(
    ...
    function renderPathwayList(item: PathwayContent): React.ReactNode {
        ...
        return (
            <>
                <h2 className='py-4 text-lg font-bold dark:text-slate-200 text-slate-800 font-4xl'>
                    {name}
                </h2>
                {content?.map(({ name, slug }) => (
                    <Link
                        ...
                        className='flex items-center w-full text-sm rounded hover:bg-slate-100 dark:hover:bg-slate-800 hover:cursor-pointer'
                    >
                        <h3 className='flex items-center mx-4 my-1 text-lg text-slate-700 dark:text-gray-400'>
                            {name}
                        </h3>
                    </Link>
                ))}
